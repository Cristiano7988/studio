...
(function ($) {
			...
			for (i = 1; i <= params[el.id].sph; i++) {
				...
				for (j = 1; j <= params[el.id].spw; j++) {
					...
					if (params[el.id].links) {
						$('#' + el.id).append("<a href='" + links[el.id][0] + "' class='cs-" + el.id + "' id='cs-" + el.id + i + j + "' style='width:" + sWidth + "%; height:" + sHeight + "%; float: left; position: absolute;'></a>");
					} else {
						$('#' + el.id).append("<div class='cs-" + el.id + "' id='cs-" + el.id + i + j + "' style='width:" + sWidth + "%; height:" + sHeight + "%; float: left; position: absolute;'></div>");
					}
	...
	$.fn.coinslider.defaults = {
		width: 865, // width of slider panel
		...
		opacity: 0.1, // opacity of title and navigation
